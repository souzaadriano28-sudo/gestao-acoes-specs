...
- [x] 5.4 Implement discriminated async state/facade primitives and verify unit tests prevent simultaneous empty/error/loading states and never retry mutations automatically.
...
- [x] 5.8 Implement shared semantic table and mobile row-card pattern plus pagination; verify header associations, labels, long content and no loss of essential fields at 320 px.
- [x] 5.9 Implement hover, focus-visible, active, disabled, invalid and pending tokens for interactive components and verify contrast plus keyboard equivalence in component snapshots.

## 6. Dashboard and portfolio pages

- [x] 6.1 Implement `/dashboard` facade/page using only its read model and verify no financial metric is calculated from independent frontend lists.
- [x] 6.2 Build dashboard summary cards with conditional availability and verify zero, unavailable and positive/negative values are distinguishable.
- [x] 6.3 Build dashboard position preview, recent movements and source-status sections and verify links reach full Carteira/Operações views with preserved context.
- [x] 6.4 Implement `/carteira` filters and detailed position table/mobile cards and verify same-ticker positions remain separated by broker.
- [x] 6.5 Add loading, empty, partial failure, stale and full error cases to both pages and verify each state with component fixtures and visual snapshots.
- [x] 6.6 Keep the operation form exclusively on `/operacoes` and verify the Dashboard exposes only a proportionate navigation CTA in desktop, tablet and mobile snapshots.

## 7. Actions and brokers pages

- [x] 7.1 Refactor `/acoes` into typed form, table and facade while preserving current API behavior; verify canonical ticker/market errors, duplicate submit protection and quote update states.
- [x] 7.2 Show quote currency, reference/fetch time, source and freshness on action rows and verify legacy source absence reads “Origem não informada”.
- [x] 7.3 Refactor `/corretoras` into typed form, table/mobile cards and facade; verify CNPJ/CEP errors, optional field omission and duplicate submit protection.
- [x] 7.4 Present business status separately from regulatory evidence and verify no legacy boolean or CNAE renders as “autorizada pela CVM”.
- [x] 7.5 Verify action and broker empty/error/stale/success states and keyboard/focus behavior in component tests.

## 8. Simulated operations

- [x] 8.1 Implement `/operacoes` with typed buy/sell form and backend-derived market/broker choices; verify only ticker, canonical market, integer quantity and broker id are submitted.
- [x] 8.2 Add accessible review step with explicit academic/no-order wording and verify no guaranteed price, execution, liquidation or custody claim appears.
- [x] 8.3 Enforce one pending submission and no optimistic financial update; verify repeated click/type change sends exactly one request and preserves displayed positions until reread.
- [x] 8.4 Handle success, business refusal, 409 conflict and unknown network outcome separately; verify field preservation, announcements and reconciliation-first actions.
- [x] 8.5 Implement paginated/filterable movement table and mobile cards and verify purchase/sale, ticker, broker, quantity, native price/currency and date remain available.
- [x] 8.6 Verify full/partial sale and successive purchase UI outcomes against deterministic backend fixtures, including unchanged average price on sale and history preservation.
